...
# Copy .git metadata for commit hash resolution
COPY .git .git
...
# Read git hash from file into env if not already set
CMD GIT_HASH=${GIT_HASH:-$(cat /app/git_hash.txt 2>/dev/null || echo unknown)}; \
    export GIT_HASH; \
