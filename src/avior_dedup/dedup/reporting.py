...
def write_summary(
    ...
    summary_lines = [
        ...
        f"  Require identical nfo year:      {'yes' if getattr(args, 'require_year_match', False) else 'no'}",
...
def sort_and_finalize_log(
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        try:
            ...
            base_no_ext, ext = os.path.splitext(log_base)
            # Always name keep files as `keep_{log_basename_no_ext}.txt` so
            # the relationship to the original dedup log is explicit.
